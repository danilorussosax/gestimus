...
import { rankWithTieBreak, effectiveStrategy } from './tiebreak.js';
...
function mapConcorso(r) {
  return {
    ...
    default_tiebreak_strategy: Array.isArray(r.default_tiebreak_strategy) ? r.default_tiebreak_strategy : null,
  };
}
function mapCommissario(r) {
  // `concorso` (singolare) è il vecchio field legacy mantenuto in schema ma non
  // più letto dalla logica. La verità è in `concorsi` (multi). Vedi migration
  // 1700000042_commissari_archivio.js.
  //
  // Importante: se `r.concorsi` è PRESENTE (anche array vuoto), è la fonte di
  // verità — un array vuoto significa "in archivio, non assegnato". NON fare
  // fallback su `r.concorso` per length=0, altrimenti dopo un disassegna che
  // svuota l'array il commissario sembra ancora assegnato (la migration popola
  // `concorsi[]` ma non svuota `concorso` legacy).
  // Il fallback su `r.concorso` resta SOLO per record pre-migration in cui
  // `r.concorsi` è undefined/null (field non esiste nello schema vecchio).
  let concorsiIds;
  if (Array.isArray(r.concorsi)) {
    concorsiIds = r.concorsi.slice();
  } else {
    concorsiIds = r.concorso ? [r.concorso] : [];
  }
  return {
    id: r.id,
    concorsi_ids: concorsiIds,
    // Backcompat: alcune view leggono ancora `concorso_id` singolo. Esponiamo il
    // primo concorso dell'array (o null se solo archivio). Da rimuovere dopo
    // refactor di palette/commissario view.
    concorso_id: concorsiIds[0] || null,
    nome: r.nome || '',
    cognome: r.cognome || '',
...
function mapFase(r) {
  return {
    ...
    tiebreak_strategy: Array.isArray(r.tiebreak_strategy) ? r.tiebreak_strategy : null,
  };
}
function mapCandidatoFase(r) {
  return {
    ...
    posizione_finale: r.posizione_finale != null ? Number(r.posizione_finale) : null,
    tiebreak_log: Array.isArray(r.tiebreak_log) ? r.tiebreak_log : null,
    ex_aequo_group: r.ex_aequo_group || null,
  };
}
...
export const db = {
  ...
  // ---------- Meta ----------
  // Imposta il ruolo "operativo" della sessione client. NON è la fonte di
  // verità del privilegio (quella è `pb.authStore.model.role`): qui blocchiamo
  // solo gli upgrade da un account di livello inferiore (es. commissario che
  // tenta di diventare admin via UI). Downgrade e null sono sempre ammessi
  // (un admin può "impersonare" un commissario per test).
  ...
  async updateConcorso(id, patch) {
    ...
    if ('logo' in patch) {
      ...
      for (const [k, v] of Object.entries(patch)) {
        if (k === 'logo') continue;
        // I campi JSON (array/object) vanno serializzati esplicitamente,
        // altrimenti FormData.append farebbe "[object Object]".
  ...
  async deleteConcorso(id) {
    ...
    // I commissari sono anagrafica per-tenant: PRIMA di cancellare il concorso
    // sganciamo (PATCH lato server) i commissari assegnati, perché il nuovo
    // field `concorsi[]` non ha cascadeDelete (vedi migration 1700000042).
    const impattati = state.commissari.filter(cm => Array.isArray(cm.concorsi_ids) && cm.concorsi_ids.includes(id));
    for (const cm of impattati) {
      const restanti = cm.concorsi_ids.filter(x => x !== id);
      // JSON body: PB SDK gestisce correttamente array vuoto su multi-relation.
      try { await pb.collection('commissari').update(cm.id, { concorsi: restanti }); }
      catch (e) { console.warn('disassegna commissario from concorso to delete failed:', e?.message); }
    }
    ...
    state.commissari = state.commissari.map(cm => {
      if (!Array.isArray(cm.concorsi_ids) || !cm.concorsi_ids.includes(id)) return cm;
      const restanti = cm.concorsi_ids.filter(x => x !== id);
      return { ...cm, concorsi_ids: restanti, concorso_id: restanti[0] || null };
    });
    ...
    // Se il commissario corrente non è più assegnato ad alcun concorso, esce dal
    // ruolo. Con il nuovo modello anagrafica (migration 1700000042) un commissario
    // può essere stato sganciato solo da questo concorso e restare assegnato ad
    // altri — in quel caso continua a vedere il ruolo commissario.
    if (state.meta.role === 'commissario' && state.meta.currentCommissarioId) {
      const cm = state.commissari.find(x => x.id === state.meta.currentCommissarioId);
      if (!cm || !Array.isArray(cm.concorsi_ids) || cm.concorsi_ids.length === 0) {
        state.meta.currentCommissarioId = null;
        state.meta.role = null;
        saveMeta();
      }
  ...
  async createFase({ concorso_id, nome, ammessi = null, data_prevista = null, scala = 10, modo_valutazione = 'autonoma', metodo_media = 'aritmetica', tempo_minuti = 0, pesi = null, criteri = null, commissari_ids = null, commissione_id = null, sezioni_ids = null, tiebreak_strategy = null }) {
    ...
    const data = {
      ...
      stato: 'PIANIFICATA',
      tiebreak_strategy: Array.isArray(tiebreak_strategy) ? tiebreak_strategy : null,
    };
  ...
  async updateFase(id, patch) {
    ...
    if ('tiebreak_strategy' in data && !Array.isArray(data.tiebreak_strategy)) data.tiebreak_strategy = null;
  ...
  getFaseCommissariIds(fase) {
    ...
    // Fallback: tutti i commissari assegnati al concorso della fase.
    return state.commissari
      .filter(c => Array.isArray(c.concorsi_ids) && c.concorsi_ids.includes(fase.concorso_id))
      .map(c => c.id);
  },
  ...
  // ---------- Commissari ----------
  // Tutti i commissari del tenant (archivio).
  archivioCommissari() {
    return state.commissari.slice().sort((a, b) => {
      const k = `${a.cognome || ''} ${a.nome || ''}`.toLowerCase();
      const l = `${b.cognome || ''} ${b.nome || ''}`.toLowerCase();
      return k.localeCompare(l, 'it');
    });
  },
  // Commissari assegnati a uno specifico concorso (subset dell'archivio).
  commissariByConcorso(concorso_id) {
    return state.commissari.filter(c => Array.isArray(c.concorsi_ids) && c.concorsi_ids.includes(concorso_id));
  },
  // Crea un nuovo record anagrafica. Se `concorsi_ids` è vuoto/omitted il record
  // finisce solo in archivio (non assegnato a nessun concorso). Per backcompat
  // si accetta ancora `concorso_id` singolo: viene mappato in `concorsi_ids=[id]`.
  async createCommissario({ concorso_id = null, concorsi_ids = null, nome, cognome = '', specialita = '', email = '', telefono = '', data_nascita = null, nazionalita = '', foto = null, cv = null, bio = '', is_presidente = false }) {
    const concorsi = Array.isArray(concorsi_ids) && concorsi_ids.length
      ? concorsi_ids.slice()
      : (concorso_id ? [concorso_id] : []);

    // `is_presidente` è legacy globale: con il nuovo modello la presidenza vive su
    // `commissioni.presidente` (migration 1700000035). Manteniamo il flag solo se
    // l'utente lo passa esplicitamente, senza più demote logic.
    const fd = new FormData();
    // `concorso` (legacy singolare): non appendiamo affatto — la verità è in
    // `concorsi[]`. Appendere stringa vuota può confondere PB sul relation.
    // `concorsi` multi-relation: appendere id ripetutamente. Se vuoto, NON
    // appendere — il field è opzionale (minSelect 0), il default è array vuoto.
    // (Appendere '' provoca validation error in PB 0.21.x).
    for (const id of concorsi) fd.append('concorsi', id);
    fd.append('nome', nome);
    fd.append('cognome', cognome || '');
  ...
  // Assegna un commissario esistente dell'archivio a un concorso (se non già
  // assegnato). Operazione idempotente.
  // Importante: per scrivere multi-relation senza upload file USIAMO JSON body,
  // non FormData. PB 0.21.x con FormData multipart su multi-relation soffre di
  // edge case (array vuoto interpretato come ['']; replace inconsistente). Con
  // JSON il SDK invia `application/json` e PB sostituisce l'array correttamente.
  async assegnaCommissarioAConcorso(commissario_id, concorso_id) {
    const com = state.commissari.find(c => c.id === commissario_id);
    if (!com) throw new Error('Commissario non trovato in archivio');
    if (!concorso_id) throw new Error('Concorso non specificato');
    if (com.concorsi_ids.includes(concorso_id)) return com;
    const nuovi = com.concorsi_ids.concat(concorso_id);
    const rec = await pb.collection('commissari').update(commissario_id, { concorsi: nuovi });
    const updated = mapCommissario(rec);
    const idx = state.commissari.findIndex(c => c.id === commissario_id);
    if (idx >= 0) state.commissari[idx] = updated;
    notify();
    return updated;
  },
  // Rimuove un commissario da uno specifico concorso (lasciandolo in archivio
  // e negli altri concorsi). Pulisce anche le presenze in commissioni del
  // concorso e i riferimenti in fasi.commissari_ids di quel concorso.
  async disassegnaCommissarioDaConcorso(commissario_id, concorso_id) {
    const com = state.commissari.find(c => c.id === commissario_id);
    if (!com) throw new Error('Commissario non trovato');
    if (!com.concorsi_ids.includes(concorso_id)) return com;

    // 1. Aggiorna concorsi[] del commissario (JSON body → array vuoto gestito ok).
    const restanti = com.concorsi_ids.filter(id => id !== concorso_id);
    const rec = await pb.collection('commissari').update(commissario_id, { concorsi: restanti });
    const updated = mapCommissario(rec);
    const idx = state.commissari.findIndex(c => c.id === commissario_id);
    if (idx >= 0) state.commissari[idx] = updated;

    // 2. Cleanup: rimuovi il commissario dalle commissioni di QUEL concorso e
    // dai fasi.commissari_ids di QUELLO stesso concorso. Le valutazioni storiche
    // restano (puntano sempre all'anagrafica, indipendentemente dall'assegnazione).
    const commsCorso = state.commissioni.filter(c => c.concorso_id === concorso_id);
    for (const cm of commsCorso) {
      let changed = false;
      let members = cm.commissari_ids.slice();
      if (members.includes(commissario_id)) {
        members = members.filter(id => id !== commissario_id);
        changed = true;
      }
      let presidente = cm.presidente_id;
      if (presidente === commissario_id) { presidente = null; changed = true; }
      if (changed) {
        await pb.collection('commissioni').update(cm.id, {
          commissari: members,
          presidente: presidente || null,
        });
        cm.commissari_ids = members;
        cm.presidente_id = presidente;
      }
    }
    const fasiCorso = state.fasi.filter(f => f.concorso_id === concorso_id);
    for (const fase of fasiCorso) {
      if (!Array.isArray(fase.commissari_ids)) continue;
      if (!fase.commissari_ids.includes(commissario_id)) continue;
      const nuovi = fase.commissari_ids.filter(id => id !== commissario_id);
      await pb.collection('fasi').update(fase.id, { commissari_ids: nuovi });
      fase.commissari_ids = nuovi;
    }
  ...
  async updateCommissario(id, patch) {
    // Nota: `is_presidente` è il flag legacy (vedi migration 1700000035 che ha
    // spostato la presidenza su `commissioni.presidente`). Con il nuovo modello
    // anagrafica (migration 1700000042) il commissario può essere in più concorsi,
    // quindi il flag globale non ha più significato a livello di concorso. Lo
    // accettiamo nel patch ma non eseguiamo più demote logic — la presidenza
    // effettiva è gestita per-commissione.
    const fd = new FormData();
...
async riordinaCandidatiFase(faseId, idsInOrder) {
  ...
  async concludiFase(faseId) {
    // 1) Congelamento classifica con cascata tiebreak PRIMA di settare CONCLUSA:
    //    se qualcosa va storto qui, la fase resta IN_CORSO (errore visibile).
    //    Dopo che CONCLUSA è settato, la valutazioni.pb.js hook blocca update.
    const fase = state.fasi.find(x => x.id === faseId);
    const concorso = fase ? state.concorsi.find(c => c.id === fase.concorso_id) : null;
    if (fase && concorso) {
      try { await this._freezeRanking(fase, concorso); }
      catch (e) {
        console.error('freezeRanking failed:', e);
        // Non blocchiamo la chiusura della fase: meglio chiudere senza
        // classifica congelata che lasciare lo stato bloccato. La UI farà
        // fallback al calcolo live se posizione_finale è null.
      }
    }
  ...
  // Applica la cascata di rottura della parità e scrive posizione_finale,
  // tiebreak_log e ex_aequo_group su ogni candidati_fase. Chiamato da
  // concludiFase prima di marcare la fase CONCLUSA.
  async _freezeRanking(fase, concorso) {
    const cfs = this.candidatiFaseList(fase.id);
    if (cfs.length === 0) return;
    // Presidente della commissione assegnata alla fase (se c'è).
    const presidente = this.getPresidenteForFase(fase);
    // Costruisci i row per il ranker: per ogni cf, calcola la media aggregata
    // e raccogli le valutazioni del candidato (servono al voto del presidente
    // e alla scomposizione per criterio).
    const rows = cfs.map(cf => {
      const cand = state.candidati.find(c => c.id === cf.candidato_id);
      const valutazioni = state.valutazioni.filter(v => v.candidato_fase_id === cf.id);
      const media = mediaCandidato(valutazioni, fase);
      return { cf, cand, media, valutazioni };
    });
    const strategy = effectiveStrategy(fase, concorso);
    const refDate = fase.data_prevista || new Date().toISOString();
    const ranked = rankWithTieBreak(rows, fase, {
      presidenteId: presidente?.id || null,
      refDate,
      allCandidati: state.candidati,
      strategy,
    });
    // Audit aggregato: una sola entry con il riassunto della cascata applicata
    // (chi è ex aequo, quanti tiebreak), per non riempire l'audit log.
    const exAequoGroups = new Set(ranked.filter(r => r.ex_aequo_group).map(r => r.ex_aequo_group));
    const tieBreaksApplied = ranked.filter(r => Array.isArray(r.tiebreak_log) && r.tiebreak_log.length > 1).length;
    if (exAequoGroups.size > 0 || tieBreaksApplied > 0) {
      this.audit('tiebreak.applied', {
        targetType: 'fase', targetId: fase.id, targetLabel: fase.nome, concorsoId: fase.concorso_id,
        payload: {
          strategy: strategy.map(s => `${s.key}:${s.enabled ? 'on' : 'off'}`).join(','),
          ex_aequo_groups: exAequoGroups.size,
          candidati_in_tiebreak: tieBreaksApplied,
        },
      });
    }
    // Persisti su PB e nello state locale.
    for (const r of ranked) {
      const patch = {
        posizione_finale: r.posizione_finale,
        tiebreak_log: r.tiebreak_log || [],
        ex_aequo_group: r.ex_aequo_group || '',
      };
      try {
        const updated = await pb.collection('candidati_fase').update(r.cf.id, patch);
        const idx = state.candidati_fase.findIndex(x => x.id === r.cf.id);
        if (idx >= 0) state.candidati_fase[idx] = mapCandidatoFase(updated);
      } catch (e) {
        console.warn('freezeRanking: update cf failed', r.cf.id, e?.message || e);
      }
    }
    notify();
  },
  ...
  async seedDemo() {
    ...
    const commsCorso = state.commissari.filter(x => Array.isArray(x.concorsi_ids) && x.concorsi_ids.includes(c.id));
